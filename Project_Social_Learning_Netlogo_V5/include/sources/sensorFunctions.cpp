 			...
 			while(m < agentsNumber)
 			{
 				...
 				for(int j = 0; j < SENSOR_NUMBER; j++)
 				{
 					if(m < agentsNumber)
 					{
 						...
 						if(onSight){myArray[i][j].setDetection(1); }
 						
 						if(!onSight)
 						{
 							border = myArray[i][j].checkSighttoBorder();
 						
 							if(border){myArray[i][j].setDetection(1); }
 							 ...
 							 Agent myObject[], int objNumber)
 {
 	...
 	for (int i = 0; i < objNumber; i++)
 	{
 		for (int j = 0; j < SENSOR_NUMBER; j++)
 		{
 			...
 			cout << "object on sight? " << onSight << endl;
 ...
 *---------------------------------------------------------------------------*/
 void borderDetection(Sensor mySensor[][SENSOR_NUMBER], int agent_i)
 {
 	...
 	for (int j = 0; j < SENSOR_NUMBER; j++)
 	{
 		...
 		if(onSight){mySensor[agent_i][j].setDetection(1); }
 		cout << "sigth to border? " << onSight << endl;
...
int signalTranslation(float myArray[], int lenght)
{
    ...
    for (int l = 0; l < lenght; l++)
    {
        ...
        { 
			...
			if(myArray[l] < 0) { cout << "negative sensor detection value: " << myArray[l] << " in position " << l << endl; cin.get();}
			if(myArray[l] > 0 && myArray[l] < 1) { cout << "float sensor detection value: " << myArray[l] << " in position " << l << endl; cin.get();}
		}
